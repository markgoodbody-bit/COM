...
import { receiptResponseEvaluation } from './receipt-response-test';
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.hostname !== '127.0.0.1' || url.search || request.headers.has('origin') ||
        request.method !== 'POST' || !['/run', '/lost-ack', '/correction', '/clear-note', '/receipt-response'].includes(url.pathname) ||
    ...
    if (url.pathname === '/receipt-response') return receiptResponseEvaluation(env);
