...
EvidenceWatch current private main is `5cb54a07ed75de083b85b1113f829ae4bd5e4a12` with 54 deterministic tests green on both Windows and Ubuntu CI. PR #7 added the CSL-JSON handoff; PR #8 repaired malformed DOI fallback; PR #9 added the synthetic-content / real-format restart-and-correction witness; PR #12 added a predeclared shadow-mode research pilot protocol; PR #13 added its deterministic offline scorer. Preserve: **CSL HANDOFF != LIVE ZOTERO/READCUBE INTEGRATION** and **SYNTHETIC WITNESS != RESEARCHER VALIDATION**. The NVIDIA submission remains bound to the earlier frozen head `e924b0de15ccaa1255bfdb80685f60f1a60172e9`.

Research demo receipt: `coordination/build_ledger/EVIDENCEWATCH_SELECTABLE_RESEARCH_DEMO_20260925.md`. Optional Digital Science recording script: `coordination/resources/DIGITAL_SCIENCE_RESEARCH_DEMO_RECORDING_20260925.md`. Judge rehearsal: `coordination/resources/DIGITAL_SCIENCE_CATALYST_2026_JUDGE_REHEARSAL_20260925.md`. Likely post-gate form copy: `coordination/resources/DIGITAL_SCIENCE_CATALYST_2026_FORM_FIELD_PACK_20260925.md`. Windows/cross-platform repair receipt: `coordination/build_ledger/EVIDENCEWATCH_WINDOWS_CI_REPAIR_20260926.md`.

Latest stronger-owner pass removes mechanism novelty: Cochrane owns important retraction/correction-to-review paths; Refract owns reproducible source-change events; AIEP P170 owns evidence-dependency/cascade architecture; ReadCube/scite own substantial workflow/monitoring surfaces. EvidenceWatch survives only as a workflow-integration/burden hypothesis. Current main `5cb54a07ed75de083b85b1113f829ae4bd5e4a12` includes the predeclared shadow-mode pilot protocol + deterministic offline scorer, PR #14's Windows file-URL/cross-platform CI repair, and documentation-only real-workflow/preprint-successor owner subtraction; current main run `36235265812` SUCCESS. No live pilot, research partner, provider call or operational reliance is implied.

The next evidential step is not more feature work. A **public historical** workflow specimen is now preserved at `coordination/build_ledger/EVIDENCEWATCH_PUBLIC_WORKFLOW_SPECIMEN_20260926.md`: the University of Bern living review explicitly checked preprints for later publication and re-extracted changed data; a concrete Lombardi preprint -> final-publication episode changed the review extraction from the earlier 41/138 state to 17/139 and fed Q1 synthesis. Current EvidenceWatch already represents the authority/succession pattern; **no code delta earned**. This confirms the maintenance task exists but also that competent existing practice already handles it. The stronger gate remains one willing current team's completed workflow + actual burden baseline. No partner is established and no contact is authorised.
...
D087 is the current public door: maintained source `028068961dd8814e639917afcde9bb2cef7b639b`, published `gh-pages@723b07f719adacbca14cd81c61a54bcbf0c58a41`, Site Preview `0.8.44`. Publication run `36198130032` completed SUCCESS with exact D086 public-head pinning and selected custom-domain byte verification; post-sync maintained CI `36198262841` is SUCCESS. D079 keeps the lighter illustrated-essay opening; D080 unified Works; D081 cleaned contextual art entrances; D082 unified the ten-room reading shell; D083 unified the appeal-example case family; D084 removed the duplicate first-action skip link; D085 makes the five appeal views human-readable first; D086 puts Explore's existing ten-question map before its preserved six-section orientation; D087 repairs only the human-facing current TRACE/ME source routes while preserving the historical node-source basis. Leave a Mark / Remark42 remains behind human/controller/legal/production-topology choices.

D087 was earned by direct published-byte inspection: human-facing current-source links had drifted from the machine source map. COM #491 is merged after exact-head CI. No TRACE/ME semantics, node text, graph relation or reader-benefit claim changed. Publication receipt: `coordination/build_ledger/PSFH_D087_PUBLICATION_20260925.md`.
