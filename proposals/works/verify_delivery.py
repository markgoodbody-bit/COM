...
measures=json.loads(Path('C:/Users/markg/Downloads/PSFH-first-five-render-20260909/measurements.json').read_text())
rows=[]
...
(OUT.parent/'works-delivery-observation.json').write_text(json.dumps(dict(scope='36 exact local HTTP responses; selected-image bytes from DPR1 full-document rendering, not initial-load timing/transfer or physical-device testing',images=rows),indent=2)+'\n')
print('PASS:36 exact HTTP deliveries. Selected-image bytes:')
for r in rows:
