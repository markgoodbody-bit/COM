...
- proposal hard-limit: 1,500 words; current exact proposal-body count after the 26 September owner-test repair is **1,420 whitespace-delimited words / 1,475 word-like tokens**; both local methods remain below the ceiling and final editor recount is still required;
...
- **RETROSPECTIVE V2 PIPELINE FROZEN / HOSTED NO-SPEND GREEN / NOT RUN LIVE** — Brierley owner dataset pinned at `a07c570c…`; v1 preserved as a pre-run source-integrity failure; executable v2 = all 22 owner-labelled `Highest_change >= 2` abstract pairs plus 22 clean reconstructable unique same-stratum nearest-date `Highest_change == 0` controls; max posting-date gap 15d, median 5d; verifier + blinded packet builder + fail-closed execution harness + conservative unblinding scorer preserved under `research/evidencewatch_retrospective/`; hosted raw-owner run `36260538589` SUCCESS; packet SHA `f12762d4…`, key SHA `75c64ca3…`; full 22+22 lexical AUCs 0.799587 / 0.795455 / 0.793388; no provider/model call, no result, no user validation.
