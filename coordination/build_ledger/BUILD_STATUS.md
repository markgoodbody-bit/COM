...
- **RETROSPECTIVE V2 PIPELINE + DECISION RULE FROZEN / HOSTED NO-SPEND GREEN / NOT RUN LIVE** — Brierley owner dataset pinned at `a07c570c…`; v1 preserved as a pre-run source-integrity failure; executable v2 = all 22 owner-labelled `Highest_change >= 2` abstract pairs plus 22 clean reconstructable unique same-stratum nearest-date `Highest_change == 0` controls; max posting-date gap 15d, median 5d; verifier + blinded packet builder + fail-closed execution harness + conservative unblinding scorer preserved under `research/evidencewatch_retrospective/`; hosted raw-owner run `36260538589` SUCCESS; packet SHA `f12762d4…`, key SHA `75c64ca3…`; full 22+22 lexical AUCs 0.799587 / 0.795455 / 0.793388; predeclared route: failures -> inconclusive, lexical weak dominance -> narrow/stop semantic-value claim, otherwise real-workflow falsification only; no provider/model call, no result, no user validation.
