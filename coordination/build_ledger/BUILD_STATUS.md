...
Useful bounded questions may be explored, but do not turn them into parallel programmes:
...
Current no-delta conclusions already earned include:
...
- `affected parties` in the seed is a wording candidate, not enough by itself to justify another edition.

Detailed reasoning/history belongs in change logs, issue/PR records, evidence files or `FRAMEWORK_HEAD.md` when still consequential—not in this build ledger.
