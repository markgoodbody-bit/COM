...
- **RETROSPECTIVE CHALLENGE + BLINDING + RUN CONTRACT + HARNESS + UNBLIND SCORER FROZEN / NOT RUN** — Brierley owner dataset pinned at `a07c570c…`; all 22 owner-labelled `Highest_change >= 2` abstract pairs plus 22 unique same-stratum nearest-date `Highest_change == 0` controls; max posting-date gap 5 days, median 2; manifest + offline verifier + blinded packet builder + fail-closed execution harness + frozen conservative unblinding scorer preserved under `research/evidencewatch_retrospective/`; exact pinned execution semantics at `coordination/resources/EVIDENCEWATCH_RETROSPECTIVE_RUN_CONTRACT_20260926.md`; trivial lexical baseline complete-case AUC ~0.78–0.79; no provider/model call, no result, no user validation.
