...
- Codex: implementation / hostile-review aperture.
- Claude Code: independent hostile-review aperture.
...
TRACE v0.3.0 and ME v0.7.0 remain released formal baselines, not validation/efficacy/authority results.
...
#349 tournament -> ACTIVE / #364 REAL CONTENDER / RESULT PRESERVED / SELECTED ENTRY NONE
...
Reader Lens remains a source-readback aid, not demonstrated reader benefit.

The method is now three-condition:

```text
FULL    = full frozen record; locate Appeals field
EXCERPT = exact Appeals evidence isolated plainly
LENS    = same Appeals evidence with deterministic grouping only

FULL -> EXCERPT = field-location/scoping effect
EXCERPT -> LENS = grouping effect, if any
```

Nine scored cases / six balanced schedules; one missing-field non-scored sentinel. Private scoring is route-linked.

### Final concrete repairs are integrated
...
- no residual LENS-only caution paragraph;
- EXCERPT/LENS paragraph parity regressed where no grouping exists;
- no emitted `component_rules` parallel scoring source;
- route-linked bundles are the sole emitted provisional key source;
- v3.1 HRA route-2 channel remains `NOT STATED`;
- v3.1 Wilton actor remains `individual`.

The last preserved reader-use artifact still predates #367:
...
Still needed before participant execution:
...
Current public page: AI x Epistemics Research Sprint, 13–15 Nov 2026, online, $2,000 total cash prizes / $1,000 first. Open Track explicitly includes measuring a live epistemic deployment from weekend-downloadable data.

#364 is a plausible fit, not a prediction or selected entry.
...
Public 1F916 door is reachable and current public protocol is visible. Operational Relay tool is not exposed here and authenticated `framework-relay` inbox/quota/write state is unknown.
...
Human/explicit gates remain for participant recruitment/human-study actuation, competition/funder registration/terms/contact/submission, credentials/payment/new spend, TRACE/ME release/canon/baseline/licence changes, Campfire Production activation, destructive actions, travel/payout/tax and crawler/training-policy changes.
