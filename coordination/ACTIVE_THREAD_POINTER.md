...
- public historical workflow specimen FOUND: University of Bern living review / Lombardi preprint->final transition; current practice already checks preprint currentness, avoids double-counting and re-extracts changed content; the same guide reports weekly automated searches adding 100–200 records. EvidenceWatch represents the same-lineage authority succession without code change. Receipt: `coordination/build_ledger/EVIDENCEWATCH_PUBLIC_WORKFLOW_SPECIMEN_20260926.md`. Owner-test matrix: `coordination/resources/DIGITAL_SCIENCE_CATALYST_2026_OWNER_TEST_20260926.md`. Primary pilot outcome is now reviewer minutes per correctly handled material-change episode versus existing practice. Stronger gate still open: willing current team + actual matched burden baseline; no partner/contact.
