...
function graphIds(index) {
  ...
  if (new Set(ids).size !== ids.length || ids.some(id => typeof id !== 'string' || !/^[a-z]+$/.test(id))) throw Error('Invalid or duplicate graph node id');
  return ids;
}
...
export function renderHumanMap(html, index, records) {
  ...
  if (navs.length !== 1 || (html.match(/<nav\b[^>]*aria-label="Optional routes"/g) ?? []).length !== 1) throw Error('Expected one Explore optional-routes list');
  const ids = graphIds(index);
  const idSet = new Set(ids);
  const links = simpleLinks(navs[0][1]);
  if (new Set(links.map(link => link.href)).size !== links.length) throw Error('Duplicate Explore route');
  ...
  for (const link of links) {
    ...
    if (!match) {
      if (link.href.startsWith('nodes/')) throw Error('Unsupported Explore node route: ' + link.href);
      otherLinks.push(link); continue;
    }
    const id = match[1];
  ...
  const items = index.nodes.map(item => {
    ...
    return `<li data-reading-node="${escape(item.id)}"><p class="map-topic">${escape(record.title)}</p><h3><a href="nodes/${escape(item.id)}.html">${escape(item.question)}</a></h3><details><summary aria-label="From here: ${escape(record.title)}">From here</summary><ul>${fromHere}</ul></details></li>`;
  }).join('');
  ...
  // Keep the source orientation inspectable, but let a visitor reach the map first.
  ...
  const opening = '<main><h1>Explore</h1><p>What are you trying to understand, change, or keep possible? Begin with a question below.</p>';
  const orientation = `<details class="explore-orientation"><summary>About this reading space</summary>${introduction[1]}</details>`;
  return html.replace(introduction[0], opening).replace(navs[0][0], replacement + orientation);
}
...
export async function writeHumanMap(sourceRoot, outputRoot) {
  ...
  // Validate identifiers before using them as local file paths.
  const ids = graphIds(index);
  const records = {};
  for (const id of ids) records[id] = JSON.parse(await readFile(path.join(sourceRoot, 'explore/nodes', id + '.json'), 'utf8'));
