...
const attempts=Number(process.env.PSFH_VERIFY_ATTEMPTS || 18);
...
for(const project of inventory.projects || []){
  for(const file of project.files || []){
    for(const field of ['current','snapshot']){
      ...
      targets.push({label:`${project.id}:${file.path}:${field}`,path:file[field],expected});
    }
  }
}

for(const path of ['/index.html','/manifest.json','/resources/inventory.json','/llms.txt','/packet.md','/changes.md','/changes.html']){
...
for(let attempt=1; attempt<=attempts; attempt++){
  ...
  for(const target of targets){
    try{
      ...
      if(actual!==target.expected) errors.push(`${target.label}: sha256 ${actual} != ${target.expected}`);
    }catch(error){
      errors.push(`${target.label}: ${error?.message || String(error)}`);
    }
  }
  if(errors.length===0){
    console.log(`LIVE_PSFH_D072_VERIFIED targets=${targets.length} attempt=${attempt}`);
    process.exit(0);
  }
  lastErrors=errors;
...
console.error('LIVE_PSFH_D072_VERIFY_FAILED');
