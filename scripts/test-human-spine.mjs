...
test('D034 changes only compact arrival, its map and HTML view, manifest and history', async () => {
  const expected = ['changes.html','changes.md','explore/map.json','explore/start.json','manifest.json','read/start.html'];
  const changed = [];
  let count = 0;
  for (const entry of await readdir('out',{recursive:true,withFileTypes:true})) {
    ...
    const before = execFileSync('git',['show','1aa0bfaba1f624519496b94d15b879a779c040da:'+file],{maxBuffer:20*1024*1024});
