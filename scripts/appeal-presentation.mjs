// Presentation of two controlled source pages, not a change to the example.
const escape = value => value.replaceAll('&', '&amp;').replaceAll('<', '&lt;').replaceAll('>', '&gt;').replaceAll('"', '&quot;').replaceAll("'", '&#x27;');
const decode = value => value.replaceAll('<br>', '\n').replaceAll('&quot;', '"').replaceAll('&#x27;', "'").replaceAll('&lt;', '<').replaceAll('&gt;', '>').replaceAll('&amp;', '&');

export function appealPresentation(html, route) {
  if (!['explore/example/case.html', 'explore/example/entry.html'].includes(route)) return html;
  ...
  const expected = isCase ? ['Status','Basis','Facts','Unknowns','Boundary','Rendering note'] : ['Status','Provenance','Offer','Shared case','Boundary','Reading boundary','Rendering note'];
  ...
  const title = isCase ? 'An appeal that comes later' : 'One situation, several viewpoints';
  const formats = `<p><a href="${isCase ? 'case' : 'entry'}.md">Complete source text</a> · <a href="${isCase ? 'case' : 'entry'}.json">JSON</a></p>`;
