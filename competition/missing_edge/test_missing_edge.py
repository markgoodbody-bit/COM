...
class MissingEdgeTests(unittest.TestCase):
    ...
    def test_manifest_has_all_conditions_and_stable_counts(self):
        ...
        self.assertEqual(len(manifest["requests"]), 6 * 5 * 3)
    ...
    def test_missing_edge_does_not_encode_independence(self):
        case = self.cases()["cases"][0]
        payload = mod.build_payload(case, "missing_edge_graph")
        self.assertEqual(payload["provenance"]["relations"], [])
        self.assertEqual(payload["provenance"]["graph_completeness"], "not_asserted")
    ...
    def test_aggregate_primary_delta_has_documented_direction(self):
        rows = [
        ...
        self.assertEqual(report["primary_intervention_delta_false_independence"], 1.0)
