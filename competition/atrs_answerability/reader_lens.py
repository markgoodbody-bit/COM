...
.controls {{ position: sticky; top: 0; z-index: 10; background: rgba(245,245,245,.96); padding: .75rem 0; display: grid; gap: .5rem; grid-template-columns: 2fr repeat(2, minmax(150px, 1fr)); }}
...
@media (max-width: 760px) {{ .controls {{ grid-template-columns: 1fr; position: static; }} .card header {{ display: block; }} .source-link {{ display: inline-block; margin-top: .5rem; }} }}
</style>
</head>
<body>
<main>
...
<p><strong>{len(rows)}</strong> frozen records; <strong>{annotated}</strong> records carry optional evidence-bound exploratory annotations.</p>
<ul>{ceilings}</ul>
</section>
<section class="controls" aria-label="Reader filters">
<input id="q" type="search" placeholder="Search tool / organisation">
...
function apply() {{
  ...
  for (const card of cards) {{
    const ok = (!needle || card.dataset.title.includes(needle) || card.dataset.labels.includes(needle)) &&
...
for (const el of [q, appeals, contact]) el.addEventListener('input', apply);
apply();
</script>
</body>
</html>'''
