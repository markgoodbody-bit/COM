...
$expectedPackage =
    '8a580d6c90d60f88ffe86acc8048711024748ce56023f117c95b4f0e5c425b43'

$expectedTarget =
    'c27d779c3cf09fff389d74842add4b1ba4d2c3150ceccafc199f3beb2d3ec080'

$expectedFilename =
    'CAMPFIRE_SQUARE_UPDATE_v0.5.3_FROM_v0.5.2_REQUESTED_READING_WORKING_CANDIDATE_R3.zip'

$receiptUrl =
    'https://raw.githubusercontent.com/markgoodbody-bit/COM/a82f8792f6e49a96813cd917689d2c5336f11e98/CAMPFIRE_SQUARE_UPDATE_RECEIPTS/8a580d6c90d60f88ffe86acc8048711024748ce56023f117c95b4f0e5c425b43.json'
...
$packages = @(
    Get-ChildItem `
        ...
        -File `
        -Filter 'CAMPFIRE_SQUARE_UPDATE_v0.5.3_FROM_v0.5.2_REQUESTED_READING_WORKING_CANDIDATE_R3*.zip' `
...
if ($packages.Count -eq 0) {
    throw @"
R3 PACKAGE NOT FOUND OR HASH DOES NOT MATCH.
...
Download R3 into Downloads and run this block again.
"@
}
...
function Read-Utf8TextFromUri([string]$Uri) {
    ...
    $request.UserAgent = 'Campfire-Square-v0.5.3-R3-bootstrap'
...
if (
    ...
    [int]$receipt.candidate_revision -ne 3 -or
    ...
    throw 'Pinned COM publication receipt does not describe this R3 transition. Nothing changed.'
}
...
$stage = Join-Path (
    [IO.Path]::GetTempPath()
) ('Campfire-Square-v053-R3-' + [guid]::NewGuid().ToString('n'))

New-Item -ItemType Directory -Force -Path $stage | Out-Null

$backupRoot = Join-Path $root (
    'Backups\v0.5.3-R3-bootstrap-' +
...
try {
    ...
    if (
        ...
        [int]$manifest.candidate_revision -ne 3 -or
    ...
    if (@($parseErrors).Count -gt 0) {
        ...
        throw "v0.5.3 R3 has $(@($parseErrors).Count) parser error(s)."
    }
    ...
    $migration = @"
CAMPFIRE SQUARE v0.5.3 R3 BOOTSTRAP
...
"@
    ...
    Write-Host ''
    Write-Host 'v0.5.3 R3 INSTALL FAILED CLOSED'
...
Write-Host ''
Write-Host 'CAMPFIRE SQUARE v0.5.3 R3 INSTALLED'
Write-Host ''
Write-Host 'Exact v0.5.2 source: PASS'
Write-Host 'R3 package SHA: PASS'
