...
# Explicit links anywhere in the fixture are excluded; the parser above also
# rejects hyperlink tags and event attributes.
if re.search(r"(?:https?://|www\.)", html, re.IGNORECASE):
    fail("human fixture unexpectedly contains explicit URL-like content")
# System-authored footer text may name local files such as register.jsonl and
# README.md. Bare-domain rejection therefore applies to the register region
# containing rendered marks, not every dot in the surrounding authored page.
...
if URL_LIKE.search(html[main_start:main_end]):
